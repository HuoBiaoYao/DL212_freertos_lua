...
//内存池(4字节对齐)
__align(4) unsigned char membase[MEM_MAX_SIZE]; //SRAM内存池
//内存管理表
unsigned short int memmapbase[MEM_ALLOC_TABLE_SIZE]; //SRAM内存池MAP
//内存管理参数	   
...
unsigned int mem_malloc(unsigned int size){  
  ...
  unsigned short int nmemb;	//需要的内存块数  
	unsigned short int cmemb=0;//连续空内存块数
  unsigned int i;
